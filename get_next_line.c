...
/*   Updated: 2024/02/10 16:18:22 by ymeziane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	str_buffer[BUFFER_SIZE + 1] = {0};
	char		*assembled_line;
	char		*line;

	if (BUFFER_SIZE < 1 || fd < 0)
		return (NULL);
	assembled_line = read_and_assemble_line(fd, str_buffer);
	if (!assembled_line)
	{
		str_buffer[0] = '\0';
		return (NULL);
	...
	update_buffer(assembled_line, str_buffer);
	return (line);
}
